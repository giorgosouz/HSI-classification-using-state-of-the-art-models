...
def get_model(name, **kwargs):
    ...
    if name == 'nn':
        ...
        kwargs.setdefault('scheduler', optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.1, patience=20, verbose=True))
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
        ...
        lr = kwargs.setdefault('learning_rate', 0.08)
        optimizer = optim.SGD(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
        kwargs.setdefault('scheduler', optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.1, patience=10, verbose=True))
        kwargs.setdefault('epoch', 100)
...
def train(net, optimizer, criterion, data_loader, epoch, scheduler=None,
          ...
          val_loader=None, supervision='full',klepsia=False):
    """
    ...
    Args:
        ...
        klepsia (optional): an test h val
    """
    ...
    ignored_labels = data_loader.dataset.ignored_labels=[0]
    accuracy, total = 0., 0.
    epoxh_metr=1
...
def test(net, img, hyperparams):
    ...
    batch_size, device = 254, hyperparams['device'] #254
