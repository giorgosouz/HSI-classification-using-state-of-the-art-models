...
DATASETS_CONFIG = {
        ...
        'train1': {
            'urls': ['http://www.ehgu.ges/ccwintco/uploads/7/72/Botswana.mat',
                     'http://www.ehgu.es/ccwintco/uploads/5/58/Botswana_gt.mat'],
            'img': 'Botswanag.mat',
            'gt': 'Botswanag_gt.mat',
            },
        'test1': {
...
#       print("WARNING: {} is not downloadable.".format(dataset_name))
    ...
    elif dataset_name == 'train1':
        # Load the image
       # Load the image
        img = open_file("testolo.npy")

        rgb_bands = (43, 21, 11)  # AVIRIS sensor

        gt = open_file("testologt.npy")
        ...
        # Load the image
        img = open_file("testolo2.npy")

        rgb_bands = (43, 21, 11)  # AVIRIS sensor

        gt = open_file("testologt2.npy")
        label_values = ["Undefined",
                        "Dense URban Fabric",
                        "Mineral EXtraction Sites",
                        "Non Irregated Arable Land",
        ...
        # Load the image
        img = open_file('12int.npy')

        rgb_bands = (43, 21, 11)  # AVIRIS sensor

        gt = open_file("12gtint.npy")
        label_values = ["Undefined",
        ...
        # Load the image
        img = open_file('345int.npy')

        rgb_bands = (43, 21, 11)  # AVIRIS sensor

        gt = open_file("345gtint.npy")
        label_values = ["Undefined",
    ...
    elif dataset_name == 'eval3':
        # Load the image
        img = open_file("345.npy")

        rgb_bands = (43, 21, 11)  # AVIRIS sensor

        gt = open_file("345gt.npy")
        label_values = ["Undefined",
                        "High Intensity Developped",
                        "Medium/Low Int.  Developped",
                        "Deciduous, Evergreen, Mixed Forest",
        ...
        # Load the image
        img = open_file("12.npy")

        rgb_bands = (43, 21, 11)  # AVIRIS sensor

        gt = open_file("12gt.npy")
        label_values = ["Undefined",
                        "High Intensity Developped",
                        "Medium/Low Int.  Developped",
                        "Deciduous, Evergreen, Mixed Forest",
    ...
    # img = np.asarray(img, dtype="float32",copy=False)
...
#                img[:,:,i]/=q
    ...
    #           scaler.fit(img[:,:,i])  # Don't cheat - fit only on training data   
    #           img[:,:,i]=scaler.transform(img[:,:,i])
...
class HyperX(torch.utils.data.Dataset):
    ...
    def __init__(self, data, gt, **hyperparams):
        ...
        self.name = 'name of the dataset used'
...
class sHyperX(torch.utils.data.Dataset):
    """ Generic class for a hyperspectral scene """
