...
def metrics(prediction, target, ignored_labels=[], n_classes=None,label_values=None):
    ...
    # print (sklearn.metrics.accuracy_score(target,prediction))
    ...
    qq=sklearn.metrics.classification_report(target,prediction,output_dict=True,target_names=label_values)
    qq['kappa']=kappa
...
def show_results1(results, label_values=None, agregated=False):
    text = ""
...
def sample_gt(gt, train_size, mode):
    ...
    if mode == 'random':
       train_indices, test_indices = sklearn.model_selection.train_test_split(X,train_size=train_size,random_state=1 )
